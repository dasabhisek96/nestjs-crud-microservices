...
// import { User } from '@/entities/user.entity'; // Ensure this is the correct path to your User entity
// import AppDataSource from '@/entities/data-source'; // Ensure this is the correct path to your DataSource file
// import 'reflect-metadata';
// import * as bcrypt from 'bcrypt';
// import * as jwt from 'jsonwebtoken';
...
//     try {
//       const userRepository = AppDataSource.getRepository(User);
//       const user = userRepository.create(payload); // Create an instance
//       return await userRepository.save(user); // Save it to the database
//     } catch (error: any) {
//       console.error(`[UserDAO_saveUser] Error: `, error);
//       throw new Error(error?.message || 'Failed to save user');
//     }
...
//             const payload = { id: user.id, email: user.email };
//             const options = { expiresIn: '1h' as jwt.SignOptions['expiresIn'] }; // Token expiration time
//             return jwt.sign(payload, secretKey, options);
//         } catch (error: any) {
//             console.error(`[UserDAO_generateJwtToken] Error: `, error);
//             throw new Error(error?.message || 'Failed to generate JWT token');
